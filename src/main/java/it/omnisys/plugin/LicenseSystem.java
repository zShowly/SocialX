...
public class LicenseSystem {
    ...
    public LicenseSystem(String licenseKey, String validationServer, SocialX plugin) {
        this.licenseKey = licenseKey;
        this.plugin = plugin;
        this.validationServer = validationServer;
    }

    public LicenseSystem setSecurityKey(String securityKey) {
    ...
    public boolean register() {
        plugin.getServer().getConsoleSender().sendMessage(color("&b[SocialX] &aStarting License System.."));
        plugin.getServer().getConsoleSender().sendMessage(color("&b[SocialX] &aConnecting to License Server..."));
        ValidationType vt = isValid();
        if (vt == ValidationType.VALID) {
            plugin.getServer().getConsoleSender().sendMessage(color("&b[SocialX] &aThe License is Valid!"));
            plugin.getServer().getConsoleSender().sendMessage(color("&m--------&r &b&lSOCIALX LICENSE &m--------"));
            return true;
        } else {
            plugin.getServer().getConsoleSender().sendMessage(color("&cYour License is not valid!"));
            plugin.getServer().getConsoleSender().sendMessage(color("Getting " + vt.toString()));
            plugin.getServer().getConsoleSender().sendMessage(color("&cDisabling plugin!"));
