...
public class LicenseSystem {
    ...
    public boolean register() {
        log(0, color("&b[SocialX] &aStarting License System.."));
        log(0, color("&b[SocialX] &aConnecting to License Server..."));
        ValidationType vt = isValid();
        if (vt == ValidationType.VALID) {
            log(1, color("&b[SocialX] &aThe License is Valid!"));
            log(0, color("&m--------&r &b&lSOCIALX LICENSE &m--------"));
            return true;
        } else {
            log(1, color("&cYour License is not valid!"));
            log(1, color("Getting " + vt.toString()));
            log(1, color("&cDisabling plugin!"));
            log(0, "");
