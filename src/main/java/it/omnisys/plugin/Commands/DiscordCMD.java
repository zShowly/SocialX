    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(sender instanceof Player) {
            ...
            if (plugin.getConfig().getBoolean("discord")) {
                if (p.hasPermission("socialx.command.discord")) {

                    p.sendMessage(color(plugin.getMessageConfig().getString("DiscordCommandMessage").replaceAll("%discordLink%", plugin.getConfig().getString("DiscordLink"))));
                } else {
                    ...
                    if (plugin.getConfig().getBoolean("debug")) {
                        for (Player player : Bukkit.getOnlinePlayers()) {
                            if (player.hasPermission("socialx.alerts")) {
                                player.sendMessage(color(plugin.getMessageConfig().getString("AdminNotifyMessage").replaceAll("%player%", p.getName()).replaceAll("%command%", "/discord")));
                            }
                        }
                    }
                }
